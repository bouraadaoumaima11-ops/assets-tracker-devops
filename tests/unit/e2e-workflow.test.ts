...
describe("E2E CI contract", () => {
  ...
  test("disables retained artifacts for both public Demo projects", () => {
    ...
    const publicDesktopStart = config.indexOf('name: "Public Demo Desktop"');
    const publicMobileStart = config.indexOf('name: "Public Demo Mobile zh-TW"');

    expect(publicDesktopStart).toBeGreaterThan(-1);
    expect(publicMobileStart).toBeGreaterThan(publicDesktopStart);

    const publicDesktop = config.slice(publicDesktopStart, publicMobileStart);
  ...
  test("keeps the public Demo CI run out of retained Playwright reports", () => {
    ...
    const reportUploadStart = workflow.indexOf("- name: Upload Playwright report");

    expect(publicRunStart).toBeGreaterThan(-1);
    expect(reportUploadStart).toBeGreaterThan(publicRunStart);

    const publicRun = workflow.slice(publicRunStart, reportUploadStart);
