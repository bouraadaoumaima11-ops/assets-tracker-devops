...
describe("E2E CI contract", () => {
  ...
  test("runs the primary smoke suite for pull requests and master pushes without secrets", () => {
    ...
    expect(e2eJobStart).toBeGreaterThan(-1);

    const e2eJob = workflow.slice(e2eJobStart);

    expect(workflow).toMatch(/^\s{2}pull_request:\s*$/m);
    expect(workflow).toMatch(/^\s{2}push:\s*$/m);
    expect(workflow).toMatch(/^\s{4}branches:\s*\[master\]\s*$/m);
  ...
  test("allows deployed-preview secrets only for trusted Vercel URLs and commits", () => {
    ...
    expect(workflow).toContain(
      "PLAYWRIGHT_TEST_BASE_URL: ${{ needs.authorize.outputs.url }}",
    );
  ...
  test("runs the serial PostgreSQL integration suite against a dedicated test database", () => {
    ...
    expect(integrationJobStart).toBeGreaterThan(-1);

    const integrationJob = workflow.slice(integrationJobStart);

    expect(integrationJob).toContain(
      "name: PostgreSQL integration tests",
    );
    expect(integrationJob).toContain(
      "POSTGRES_DB: asset_app_asset_tracker_test",
    );
    expect(integrationJob).toContain(
  ...
  test("enables public Demo only in the isolated E2E environment", () => {
    ...
    const deployedPreviewWorkflow = read(
  ...
  test("disables retained artifacts for both public Demo projects", () => {
    ...
    const publicDesktopStart = config.indexOf(
      'name: "Public Demo Desktop"',
    );
    const publicMobileStart = config.indexOf(
      'name: "Public Demo Mobile zh-TW"',
    );

    expect(publicDesktopStart).toBeGreaterThan(-1);
    expect(publicMobileStart).toBeGreaterThan(publicDesktopStart);
    ...
    const publicMobile = config.slice(publicMobileStart);
  ...
  test("keeps the public Demo CI run out of retained Playwright reports", () => {
    ...
    const publicRunStart = workflow.indexOf(
      "- name: Run public Demo E2E tests without retained artifacts",
    );

    const reportUploadStart = workflow.indexOf(
      "- name: Upload Playwright report",
    );

    expect(publicRunStart).toBeGreaterThan(-1);
    expect(reportUploadStart).toBeGreaterThan(publicRunStart);
    ...
    expect(config).toContain(
      'process.env.E2E_PUBLIC_DEMO_NO_ARTIFACTS === "1"',
    );
    ...
    expect(publicRun).toContain(
      'E2E_PUBLIC_DEMO_NO_ARTIFACTS: "1"',
    );
    expect(publicRun).toContain(
      '--project="Public Demo Desktop"',
    );
    expect(publicRun).toContain(
      '--project="Public Demo Mobile zh-TW"',
    );
  ...
  test("keeps public Demo API assertions out of sensitive object and array diffs", () => {
    ...
    expect(publicSpec).not.toContain(
      "expect(resetAccounts).toHaveLength",
    );
    expect(publicSpec).toContain(
  ...
  test("cleans Demo users through the relation only for local disposable databases", () => {
    ...
    expect(teardown).toContain(
      'SELECT "userId" FROM "DemoWorkspace"',
    );
    expect(
      teardown.indexOf("await cleanupPublicDemoUsers()"),
    ).toBeLessThan(
  ...
  test.each(["README.md", "README.zh-TW.md"])(
    ...
    (readme) => {
      expect(read(readme)).toContain(
        "e2e.yml/badge.svg?branch=master&event=push",
      );
    },
  );
  ...
  test("documents CPU baselines, regression thresholds, and kill-switch response", () => {
    ...
    expect(deployment).toContain(
      "application compute CPU utilization",
    );
    expect(deployment).toContain(
      "24-hour pre-enable baseline window",
    );
