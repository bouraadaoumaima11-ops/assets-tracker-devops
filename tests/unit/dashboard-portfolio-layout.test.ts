...
const concentrationSource = readFileSync(
  "src/components/dashboard/concentration-card.tsx",
  "utf8",
);
...
describe("dashboard portfolio layout", () => {
  it("aligns the composition card with the desktop overview row and fills it internally", () => {
    ...
    expect(
      heatmapSource.match(
        /fillHeight && "lg:flex lg:min-h-0 lg:flex-col"/g,
      ),
    ).toHaveLength(2);
  });

  it("separates concentration from the 8/4 portfolio overview row", () => {
    const overviewStart = dashboardSource.indexOf(
    ...
    expect(concentrationSource).toContain(
  ...
  it("keeps the loading skeleton topology aligned with the dashboard", () => {
    const overviewStart = skeletonSource.indexOf(
    ...
    expect(skeletonSource).toContain(
      'data-testid="portfolio-concentration-skeleton"',
    );
  ...
  it("lays concentration out horizontally on desktop", () => {
    expect(concentrationSource).toContain(
      "lg:grid-cols-[minmax(12rem,0.3fr)_minmax(0,1fr)]",
    );
